...
function createElement() {
	...
	return normalizeVNode(preact.h.apply(undefined, args));
}
...
function cloneElement$1(element, props) {
	...
	return normalizeVNode(preact.cloneElement.apply(undefined, cloneArgs));
}
...
function shallowDiffers(a, b) {
	for (var _i4 in a) {
		if (!(_i4 in b)) return true;
	}for (var _i5 in b) {
		if (a[_i5] !== b[_i5]) return true;
	}return false;
}
...
function collateMixins(mixins) {
	var keyed = {};
	for (var _i6 = 0; _i6 < mixins.length; _i6++) {
		var mixin = mixins[_i6];
...
function bindAll(ctx) {
	for (var _i7 in ctx) {
		var v = ctx[_i7];
		if (typeof v === 'function' && !v.__bound && !AUTOBIND_BLACKLIST.hasOwnProperty(_i7)) {
			(ctx[_i7] = v.bind(ctx)).__bound = true;
		}
	}
}
...
function multihook(hooks, skipDuplicates) {
	return function () {
		var ret = void 0;
		for (var _i8 = 0; _i8 < hooks.length; _i8++) {
			var r = callMethod(this, hooks[_i8], arguments);
...
extend(Component$1.prototype = new preact.Component(), {
	...
	replaceState: function replaceState(state, callback) {
		...
		for (var _i9 in this.state) {
			if (!(_i9 in state)) {
				delete this.state[_i9];
			}
		}
	},
