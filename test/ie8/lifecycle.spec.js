...
describe('Lifecycle methods', () => {
  ...
  describe('#componentWillUpdate', () => {
      ...
      let doRender;
      let didMount = sinon.spy()
      let receiveProps = sinon.spy()
        ...
        componentWillReceiveProps () {
          receiveProps()
			...
			expect(receiveProps.calls.count()).toBe(0)

			// Rerender inner with new props
			doRender.setState({ i: doRender.state.i + 1 })
			rerender();
			expect(didMount.calls.count()).toBe(1)
		});

		it('should be called on new state', () => {
      let doRender;
      let willUpdate = sinon.spy()
			class ReceivePropsComponent extends Component {
				componentWillUpdate() {
          willUpdate()
        }
      ...
      render(<ReceivePropsComponent />, scratch)
      expect(willUpdate.calls.count()).toBe(0)

			doRender.setState({ i: doRender.state.i + 1 })
			rerender();
			expect(willUpdate.calls.count()).toBe(1)
		});
		...
		it('should be called after children are mounted', () => {
			...
			class Inner extends Component {
				componentDidMount() {
					...
					// callback is invoked.
					expect(scratch.querySelector('#inner')).toEqual(this.base);
				}
			...
			// expect(log).toEqual(['Outer updated', 'Inner mounted']);
		});
	});

  describe('#componentWillReceiveProps', () => {
    const receiveProps = sinon.spy();
		it('should NOT be called on initial render', () => {
			class ReceivePropsComponent extends Component {
				componentWillReceiveProps() {
          receiveProps()
      ...
      let doRender;
      let didMount = sinon.spy();
      let willReceiveProps = sinon.spy();
			class Outer extends Component {
				constructor(p, c) {
					super(p, c);
					this.state = { i: 0 };
          ...
          willReceiveProps()
			...
			// Initial render
      render(<Outer />, scratch);
      expect(willReceiveProps.calls.count()).toBe(0)

      // // Rerender inner with new props
      doRender.setState({ i: doRender.state.i + 1 })
			rerender();
			expect(willReceiveProps.calls.count()).toBe(1)
		});
	...
	// 			componentWillUnmount() {}
	// 		}
	// 		spyOn(Foo.prototype, 'componentDidMount');
	// 		spyOn(Foo.prototype, 'componentWillUnmount');
	// 		spyOn(Bar.prototype, 'componentWillUnmount');
